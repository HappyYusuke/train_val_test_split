...
for i in tqdm(range(int(len(label_paths) * (rate['train']/total)))):
    ...
    # 取得したファイル名はリストから削除する
    image_paths = [path for path in image_paths if not file_name+image_dot in path]
    label_paths = [path for path in label_paths if not file_name+label_dot in path]
    # コピー元とコピー先のパスを作成する
